...
import sys
# Add parent directory to path to find subtitle_styles
sys.path.insert(0, str(Path(__file__).parent.parent))

# We will need to copy the subtitle_styles folder from clip_app_1 as well
# For now, we assume it exists in the root of clip_app_2
...
class StyleLoader:
    ...
    def load_style(style_name: str, styles_path: Path = None) -> Dict:
        ...
        if styles_path is None:
            # Default to local styles.json
            styles_path = Path(__file__).parent.parent / "subtitle_styles" / "styles.json"
...
class SubtitleRenderer:
    ...
    def _render_frame(
        ...
        # Extract word texts and remove punctuation
        import string
        punctuation_to_remove = string.punctuation + "।"  # Add Hindi danda
        word_texts = []
        for w in active_words:
            text = w.get('text_roman', w.get('text', ''))
            # Remove punctuation
    ...
    def _render_glow_style(
        ...
        bottom_margin = style['layout']['safe_margins']['bottom']
        max_width = style['layout'].get('max_width', 900)
