...
class WordHighlightEffects:
    ...
    @staticmethod
    def create_color_highlight_with_glow(words: List[str],
                                       font_path: str,
                                       font_size: int,
                                       ...
                                       glow_radius: int,
                                       highlighted_word_index: int = -1,
                                       image_size: Tuple[int, int] = (1080, 200),
                                       bottom_margin: int = 50,
                                       max_width: int = 900) -> np.ndarray:
        """
        Create text with glow effect and color highlighting
        
        Args:
            words: List of words to render
            font_path: Path to font file
            font_size: Font size in pixels
            normal_color: RGB color for normal words
            highlighted_color: RGB color for highlighted word
            outline_color: RGB color for outline/glow
            outline_width: Width of outline
            glow_radius: Radius of glow blur
            highlighted_word_index: Index of word to highlight
            image_size: Output image dimensions
            bottom_margin: Margin from bottom
            max_width: Maximum width for text (scales down if exceeded)
        """
        width, height = image_size
        padding = 50
        ...
        # Calculate total width to check against max_width
        total_width = 0
        word_widths = []
        space_width = draw.textbbox((0, 0), " ", font=font)[2]
        ...
        scale_factor = 1.0
        if total_width > max_width and max_width > 0:
            scale_factor = max_width / total_width
            new_font_size = int(font_size * scale_factor)
            try:
                font = ImageFont.truetype(font_path, new_font_size)
                # Recalculate widths with new font
                total_width = 0
                word_widths = []
                space_width = draw.textbbox((0, 0), " ", font=font)[2]
                for word in words:
                ...
                pass # Keep original if reload fails
        ...
        draw = ImageDraw.Draw(img)
        for i, pos in enumerate(word_positions):
            color = highlighted_color if i == highlighted_word_index else normal_color
            
            # Draw outline if needed (sharp outline on top of glow)
            if outline_width > 0:
                for dx in range(-1, 2):
                    for dy in range(-1, 2):
